...
const task=cron.createTask('* * * * * *', async()=>{
  console.log('cron here runs every minute');
},{noOverlap:true})
...
const createSendCookie = async (res: Response, user: userData,message:string,status?:number) => {
  try {
    ...
    return res
      .status(status||200)
      .cookie('jwt', token)
      .json({ success: true, data: user,message:message,source:'db' });
  } catch (error) {
...
const verify_opt=async(req:Request,res:Response,next:NextFunction)=>{
  try {
    ...
    const userData=await Users.findOne<userData|any>({where:{id:userId,email:email}});
  ...
  // check if the user data is already present in the redis cache
...
const completeProfile=async(req:Request,res:Response,next:NextFunction)=>{
  try {
    ...
    const userData:UserDetails={...data,user_id:userId,resume:'resume.pdf'}
    ...
    // console.log(user);
    await Users.update({isProfileCompleted:true},{where:{id:userId}})
    
    // get the user details
    const user:userData=await Users.findOne<userData|any>({where:{id:userId,email:email},include:{
      model:User_Profile,
      as:'user_details'
    }});
...
const authME=async(req:Request,res:Response,next:NextFunction)=>{
  ...
  const cachedUser=await redisClient.get(cachedKey);
  console.log('redisuser',cachedUser);
  ...
  //  get the user form the database
   ...
   if(!user)
    ...
    // now set the user data in the redis cached
    await redisClient.set(cachedKey,JSON.stringify(user));
    // return the response
    console.log('corn job stopped');
    await task.destroy();
    return res.status(200).json({message:'success',data:user,source:'db'})
  } catch (error) {
    console.log(error);
   next(new AppError('Server error',500));
  }
